...
export const updateStory = async (storyId, slides) => {
  try {
    ...
    const res = await axios.put(
      `${staticUrl}/api/v1/story/update/${storyId}`,
      slides
    );
    return res.data;
  } catch (error) {
...
export const getStoryById = async (id) => {
  try {
    const res = await axios.get(
      `${staticUrl}/api/v1/story/getStoryById/${id}`
    );
    return res.data;
  } catch (error) {
...
export const likedStory = async (userId, storyId) => {
  try {
    ...
    axios.defaults.headers.common["Authorization"] = token;
    const res = await axios.put(
      `${staticUrl}/api/v1/story/likedStory/${storyId}/${userId}`
    );
    return res.data;
    console.log(res);
  } catch (error) {
...
export const bookmarkStory = async (story) => {
  try {
    ...
    axios.defaults.headers.common["Authorization"] = token;
    const res = await axios.put(
      `${staticUrl}/api/v1/story/bookmarkStory`,
      story
    );
  } catch (error) {
...
export const getbookmarks = async () => {
  try {
    ...
    axios.defaults.headers.common["Authorization"] = token;
    const res = await axios.get(
      `${staticUrl}/api/v1/story/getBookmarks`
    );
  ...
  try {
    ...
    if (filterArray.length === 0) {
      response = await axios.get(
        `${staticUrl}/api/v1/story/getAllstory?cat=${cat}&page=${page}`
      );
    } else {
      response = await axios.get(
        `${staticUrl}/api/v1/story/getAllstory?categories=${filterArray}`
      );
    }
